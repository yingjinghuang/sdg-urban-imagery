...
#   bash scripts/pretrain_rs.sh spatial <tag> [init_ckpt]
#   bash scripts/pretrain_rs.sh <objective> <tag> [init_ckpt] [resume_ckpt]
#
...
OBJECTIVE="${1:?usage: pretrain_rs.sh <self|spatial> <tag> [init_ckpt] [resume_ckpt]}"
TAG="${2:?usage: pretrain_rs.sh <self|spatial> <tag> [init_ckpt] [resume_ckpt]}"
INIT_CKPT="${3:-}"
RESUME_CKPT="${4:-}"
...
if [ -n "${INIT_CKPT}" ] && [ -n "${RESUME_CKPT}" ]; then
    echo "[pretrain-rs] use either init_ckpt or resume_ckpt, not both" >&2
    exit 2
fi
