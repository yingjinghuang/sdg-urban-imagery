...
# This cleaned entry point keeps the training behavior needed by the paper
# without experiment-tracking service dependencies or embedded credentials.
...
parser = argparse.ArgumentParser(description="MoCo-v3 street-view pretraining")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if "self" in os.path.basename(args.data):
        ...
        train_dataset = GSVDatasetSpatial(args.data, transforms.Compose(augmentation1))
...
class GSVDatasetSelf(torch.utils.data.Dataset):
...
class GSVDatasetSpatial(torch.utils.data.Dataset):
    def __init__(self, file, transform=None):
        df = pd.read_pickle(file)
        self.transform = transform
